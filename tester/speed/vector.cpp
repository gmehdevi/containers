...
#include <time.h> 
#include <iostream>
#define count 1000// dont go beyond 10000 takes too long
#ifdef FT
    #define ns ft
#else
    #define ns std
...
int main(int ac, char **av) {
    ...
    srand(atoi(av[1]));
    // vector<int>v;
    // v.insert(v.begin(), count, rand()%count);
    // for (size_t i = 0; i < count ; i++)
    //     v.erase(v.begin());     
    // for (size_t i = 0; i < count ; i++)
    //     v.insert(v.begin(),rand()%count);
...
{    
    ...
    v.insert(v.begin() + count/2, count, 42);
    end = clock();
    ...
    std::cout << "clear :"<<(double)(end - begin) / CLOCKS_PER_SEC << std::endl;
    clock_t e = clock();
    std::cout << "TOTAL :" << (double)(e - b) / CLOCKS_PER_SEC << std::endl;
